import{E as nD,F as uh,N as tD,P as th,S as ZL,T as eh,_ as Qv,a as ED,b as Xp,f as Ly,i as Cc,j as ri,l as JD,n as $D,o as EI,r as AD,v as Su,w as ch,x as YL}from"./main-IBXGZ6IW.js";import{i as lt,n as Z,r as j$1,t as K}from"./chunk-Bi1jNM8B.js";var d=`#version 300 es
in vec2 position;
in vec2 uv;
...
}`;var A=[`canvas`];var c=class t{fragmentShader=YL.required();canvasRef=ZL.required(`canvas`);renderer;program;mesh;animationFrameId;timeOffset=0;updateShader=Su(()=>{this.program.setShaders({vertex:d,fragment:this.fragmentShader()}),this.timeOffset=performance.now()});ngOnInit(){let a=this.canvasRef().nativeElement;this.renderer=new Z({canvas:a,dpr:Math.min(window.devicePixelRatio,2),alpha:!0});let e=this.renderer.gl,o=new j$1(e,{position:{size:2,data:new Float32Array([-1,-1,3,-1,-1,3])},uv:{size:2,data:new Float32Array([0,0,2,0,0,2])}});this.program=new K(e,{vertex:d,fragment:this.fragmentShader(),uniforms:{uTime:{value:0},uResolution:{value:[e.canvas.clientWidth,e.canvas.clientHeight]}},transparent:!0}),this.mesh=new lt(e,{geometry:o,program:this.program}),this.resize(),this.animate(0)}animate=a=>{this.program.uniforms.uTime.value=(a-this.timeOffset)*.001,this.renderer.render({scene:this.mesh}),this.animationFrameId=requestAnimationFrame(this.animate)};resize(){let a=this.canvasRef().nativeElement,e=a.clientWidth||window.innerWidth,o=a.clientHeight||window.innerHeight;this.renderer.setSize(e,o);let i=this.renderer.gl;this.program.uniforms.uResolution.value=[i.canvas.width,i.canvas.height]}ngOnDestroy(){cancelAnimationFrame(this.animationFrameId)}static ɵfac=function(e){return new(e||t)};static ɵcmp=EI({type:t,selectors:[[`app-shader-preview`]],viewQuery:function(e,o){e&1&&uh(o.canvasRef,A,5),e&2&&ED()},hostBindings:function(e,o){e&1&&ch(`resize`,function(){return o.resize()},Qv)},inputs:{fragmentShader:[1,`fragmentShader`]},decls:2,vars:0,consts:[[`canvas`,``]],template:function(e,o){e&1&&th(0,`canvas`,null,0)},styles:[`[_nghost-%COMP%]{width:100%;height:100%}canvas[_ngcontent-%COMP%]{width:100%!important;height:100%!important;display:block}`]})};var j=`#version 300 es
...
}`;function q(t,a){t&1&&$D(0,` Smoke `)}function B(t,a){t&1&&($D(0,` Black`),eh(1,`br`),$D(2,`hole `))}function G(t,a){t&1&&$D(0,` Noise `)}var W={noise:j,smoke:N,"black-hole":D};var E=class t{type=YL.required();fragmentShader=JD(()=>W[this.type()]);static ɵfac=function(e){return new(e||t)};static ɵcmp=EI({type:t,selectors:[[`app-shaders-page`]],hostVars:2,hostBindings:function(e,o){e&2&&AD(o.type())},inputs:{type:[1,`type`]},decls:5,vars:2,consts:[[1,`noise-container`,3,`fragmentShader`],[1,`title-section`]],template:function(e,o){if(e&1&&(eh(0,`app-shader-preview`,0),ri(1,`h1`,1),tD(2,q,1,0)(3,B,3,0)(4,G,1,0),Cc()),e&2){let i;Xp(`fragmentShader`,o.fragmentShader()),Ly(2),nD((i=o.type())===`smoke`?2:i===`black-hole`?3:4)}},dependencies:[c],styles:[`[_nghost-%COMP%]{display:flex;width:100%;min-height:100vh;--%NS%shaders-page-background: unset;--%NS%shaders-page-label-color: black;--%NS%shaders-page-label-blend-mode: unset}.smoke[_nghost-%COMP%]{--%NS%shaders-page-background: #111;--%NS%shaders-page-label-color: white}.black-hole[_nghost-%COMP%]{--%NS%shaders-page-label-blend-mode: overlay}.noise-container[_ngcontent-%COMP%]{position:fixed;background:var(--%NS%shaders-page-background);z-index:-1}.title-section[_ngcontent-%COMP%]{margin:auto;text-align:center;font-size:clamp(8rem,15vw,30rem);font-family:sans-serif;text-transform:uppercase;color:var(--%NS%shaders-page-label-color);mix-blend-mode:var(--%NS%shaders-page-label-blend-mode)}`]})};export{E as ShadersPage};